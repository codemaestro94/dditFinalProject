...
public class OwnerTradingController {
   ...
   // 검색
   @RequestMapping(value="/tradingList.do", method = RequestMethod.POST)
   public String tradingHistorySearch(TradingVO tradVO, Model model) {
   
      log.info("tradVO!!!! : " + tradVO);
      
      
      model.addAttribute("sendReceive",tradVO.getSendReceive());
      model.addAttribute("progress",tradVO.getProgress());
   ...
   @ResponseBody
   @RequestMapping(value="/tradingSearch.do", method = RequestMethod.POST)
   public ResponseEntity<List<TradingVO>> tradingSearch(TradingVO tradVO) {
	   System.out.println("tradVO : " + tradVO.getSendReceive());
	   
	   List<TradingVO> tradList = service.getSearch(tradVO);
	   
	   
	return new ResponseEntity<List<TradingVO>>(tradList, HttpStatus.OK);
   }
