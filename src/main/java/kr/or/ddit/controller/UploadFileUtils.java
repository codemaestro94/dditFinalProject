...
// bean등록을 controller로 하지않고 직접 호출해서 사용할 클래스
public class UploadFileUtils {
	...
	// 키워드 '...' 를 사용한다.
	// [사용법] 타입...변수명 형태로 사용
	// 순서대로 넘겨받고있는 yearPath, monthPath, datePath가 배열로 들어가 처리
	private static void makeDir(String uploadPath, String ...paths) {
		...
		for(String path: paths) {
			...
			// /2023/09/04와 같은 경로에 각 폴더가 없으면 각각 만들어준다.
			if(!dirPath.exists()) {
				dirPath.mkdirs();
			}
		}
